<cfcomponent>
	...
	<cffunction name="standard" description="locations page" output="Yes" access="public">
			...
			<div class="row">
				<cfset variables.dealercount=0>
				<cfloop query="Get_All_Dealers">
					<cfif len(trim(Get_All_Dealers.Link_Charity)) gt 0>
						<cfset variables.dealercount=variables.dealercount+1>
						...
						<div class="col-md-4">
							<div class="loc-dlr-div">
								...
								<cfif len(trim(Get_All_Dealers.Link_Charity)) gt 0><a class="btn btn-grey" href="#Get_All_Dealers.Link_Charity#" target="_blank"><img class="btn-icon" src="http://#cgi.server_name#/images/inventory/icon_SM_Charity_off.png"/>Visit Charity Website</a></cfif>
							</div>
						</div>
					</cfif>
					<cfif variables.dealercount mod 3 eq 0>
						</div>
						<div class="row">
					</cfif>
				</cfloop>
