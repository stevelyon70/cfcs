...
<cfif #t_clik.track_clicks# EQ 1 AND #form.voi_vin# NEQ "" >
	<cfquery datasource="Ultra10" name="g_Clk">SELECT ID, send_to_friend_click FROM Vehicle_Click_Stats WHERE VIN='#form.VOI_Vin#' </cfquery>
	<cfif #g_Clk.recordcount# NEQ 0 ><cfset g_Clk.send_to_friend_click = #g_Clk.send_to_friend_click# + 1 > 
	<cfquery datasource="Ultra10" name="U_Clk">UPDATE Vehicle_Click_Stats SET  send_to_friend_click=#g_Clk.send_to_friend_click#,last_Upd=#CreateODBCDateTime(now())#WHERE VIN='#form.VOI_Vin#'</cfquery></cfif>
</cfif> </cfoutput>    
<!--- Clicks Tracker 03-02-2016  --->
