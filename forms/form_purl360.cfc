	...
	<cffunction name="purls_default" description="display default contact us form" output="Yes" access="public">
		...
		<cfargument name="ems_instruct" default="Please enter your contact information">
		<cfargument name="ems_vehicle" default="0">
		<cfargument name="form_action" default="#GetFileFromPath(cgi.CF_TEMPLATE_PATH)#/forms/purl360_action.cfm"> 

		<cfset var Get_Form_Locations="">
		<cfset multi_purl = 0 >
		
		<cfquery name="load_cust_info" datasource="wd360">
		...
		<cfquery name="get_pident" datasource="wd360">SELECT ID, Campaign_ID, PURL_Identifier FROM dealers WHERE	dealer_ID = #load_cust_info.Dealer_ID# </cfquery>
		...
		<form id="tap_drive" class="form-horizontal" action="#arguments.form_action#" method="post">
			<fieldset>
				...
				<cfif #get_pident.PURL_Identifier# EQ 601 OR  #get_pident.PURL_Identifier# EQ 700 >
					<input type="hidden" id="lead_source" name="lead_source" value="0">						<!--- Hardcode because of Vehicle Prompt --->
				<cfelse>
				...
				<input type="hidden" id="purl_dlr" name="purl_dlr" value="382">	 
				...
				<cfelse>
					<cfif IsDefined("arguments.pass_ems") and #arguments.pass_ems# EQ 1 >
						<div class="row" ><div class="col-md-12 col-md-offset-1" align="left"><div class="body-text-bold" >#arguments.ems_instruct#</div></div></div>
				...
				<cfif ( #arguments.pass_ems# EQ 1  and #arguments.ems_vehicle# EQ 1 ) OR  ( #arguments.pass_ems# EQ 0  and #arguments.ems_vehicle# EQ 1 ) >
					 <div class="form-group">
						<div class="col-md-12 col-md-offset-1" align="left">
							<div  class="body-text-bold">Vehicle Year</div>
							<input name="voi_year" class="input-xlarge" style="border:1px solid ##EBEBEB;min-height:25px"  value="#load_cust_info.v_year# "  id="voi_year" type="text" size="10"  maxlength="10"   >
						</div>
					</div> 
					 <div class="form-group">
						<div class="col-md-12 col-md-offset-1" align="left">
							<div  class="body-text-bold">Vehicle Make</div>
							<input name="voi_make" class="input-xlarge" style="border:1px solid ##EBEBEB;min-height:25px"  value="#load_cust_info.v_make#"  id="voi_make" type="text" size="40"  maxlength="100"  >
						</div>
					</div> 
					 <div class="form-group">
						<div class="col-md-12 col-md-offset-1" align="left">
							<div  class="body-text-bold">Vehicle Model</div>
							<input name="voi_model" class="input-xlarge" style="border:1px solid ##EBEBEB;min-height:25px"  value="#load_cust_info.v_model# "  id="voi_model" type="text" size="40"  maxlength="100"  >
						</div>
					</div> 
				<cfelse>
					<input name="voi_year" value="#load_cust_info.v_year# " type="hidden"  >	
					<input name="voi_make" value="#load_cust_info.v_make# " type="hidden"  >	
					<input name="voi_model" value="#load_cust_info.v_model# " type="hidden"  >	
				</cfif>	
				<div class="form-group">
					<div class="col-md-12 col-md-offset-1"  align="left">
				...
				<div class="form-group">
					<div class="col-md-12 col-md-offset-1" align="left">
					<cfif #url.dealer_id# Neq 337 AND  #url.dealer_id# Neq 368 >
	...
	<cffunction name="form_submit_purls" description="action page for form" output="Yes" access="public">
		...
		<cfargument name="dsn" default="wd360">
...
<!--- If Email Submission, build pURL --->
		  <cfif #form.lead_source# EQ 1 >
			...
			<cfquery NAME="InsertEpurl" datasource="wd360">
				  INSERT into Customer_PURLs ( 	Campaign_ID, 
				  					...
				  					VALUES 	( 	#Form.Campaign_ID#,
												...
												#CreateODBCDate(Form.start_dt)#, #CreateODBCDate(Form.expires_dt)#,
												#CreateODBCDate(Now())#, #CreateODBCDate(Now())#,
												1 ,0, 1  )
				</cfquery>
...
<!--- Create "MyID"  --->
			...
			<cfquery datasource="wd360" name="Check_Pin">
				SELECT 	 ID, Campaign_ID, Dealer_ID, Firstname, Lastname, PURL, Destination_URL, Max_Loan_Amount, create_dt, first_visit_dt, last_visit_dt, start_dt, expires_dt, 
                      nbr_visits, active, purl_type, Address_1, Address_2, City, State, Zip, Phone, Email, v_year, v_make, v_model, authorization_nbr
				FROM         Customer_PURLs
			...
			<cfelse> 
				...
				<cfquery datasource="wd360" name="Get_S_Dealer">
				...
				<cfif Check_Pin.Recordcount gt 0>
					<cfquery datasource="wd360" name="Insert_LEAD">
						INSERT INTO	Leads
									(campaign_id,
									dealer_id,
									Date_Submitted,
							...
							VALUES
									...
									<cfqueryparam value="#Check_Pin.Phone#" cfsqltype="CF_SQL_VARCHAR" maxlength="50">,  
									...
									<cfqueryparam value="#Form.voi_make#" cfsqltype="CF_SQL_VARCHAR" maxlength="50">,
									<cfqueryparam value="#Form.voi_model#" cfsqltype="CF_SQL_VARCHAR" maxlength="100">,
				...
				<cfif variables.error eq 0 >
					...
					<cfquery datasource="wd360" name="Get_CCAP_Dealer">
					...
					<cfset variables.dlr_dir = #Get_program_Details.Dealer_Directory#>  
			 		...
			 		<cfquery name="get_subject" datasource="wd360">
						SELECT    ID, lead_email_subject,customer_email_subject, lead_email_sender,customer_email_sender
						FROM       dealers
						WHERE	dealer_id	=	#Get_CCAP_Dealer.Dealer_ID#
					</cfquery>

					<!--- set mail parameters --->
					<!--- <cfset variables.WD_BCC="wdleads@gmail.com,wdpittsburgh@hotmail.com,wdsupport@worlddealer.net"> --->
					
					<cfif #get_subject.customer_email_subject#  NEQ "" >
						<cfset variables.subject="#get_subject.customer_email_subject# from #Get_CCAP_Dealer.Dealer_Name#">
					<cfelse>
						<cfset variables.subject="Congratulations on your Bonus Trade-In Offer from #Get_CCAP_Dealer.Dealer_Name#">	
					</cfif>	
					
					 <cfif #get_subject.lead_email_subject#  NEQ "" >
						<cfset variables.lead_subject="#get_subject.lead_email_subject# from #Get_CCAP_Dealer.Dealer_Name#">
					<cfelse>
						<cfset variables.lead_subject="Bonus Trade-In Offer Claim from #Get_CCAP_Dealer.Dealer_Name#">
					</cfif>	
					 <cfif #get_subject.lead_email_sender#  NEQ "" >
						<cfset variables.from="#get_subject.lead_email_sender#">
					<cfelse>
						<cfset variables.from="thetradeoffer@thetradeoffer.com">
					</cfif>	
				...
				<!--- Leads Emails --->			
					<cfif #s_email_type# EQ 1  >
						...
						<cfinvoke component="/cfcs/emails/adf_gen" method="contact_adf" returnvariable="adf_email">
							...
							<!--- <cfinvokeargument name="lead_source" value="#Get_CCAP_Dealer.lead_source#"/> --->
				...
				<cfelse>		
							...
							<table cellpadding="0" width="70%" cellspacing="0">
								<tr>
									<td align="left" > 
										...
										Lead Source: <cfif #Get_CCAP_Dealer.lead_source# EQ 0 >PURL<cfelse>EMAIL</cfif><br>
					...
					<cfelse>
						...
						<!--- <cfmail to="#variables.recipient_list#" from="#variables.from#" subject="#variables.lead_subject#" bcc="kelly@worlddealer.net,wdsupport@worlddealer.net"  type="HTML"> --->		
						 <cfmail to="#variables.recipient_list#" from="#variables.from#" subject="#variables.lead_subject#" bcc="kelly@worlddealer.net,wdsupport@worlddealer.net,bob@worlddealer.net"  type="HTML">			
							<table cellpadding="0" width="70%" cellspacing="0">
								<tr>
									<td align="left" > 
										Name: #form.First_Name# #form.Last_Name#<br>
