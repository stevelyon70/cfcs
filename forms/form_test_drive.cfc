				...
				<div class="form-group">
					...
					<div class="col-md-6">
						<select class="form-control input-md" name="Voi_New_Used" id="Voi_New_Used" required>
							<cfif #arguments.dealer_id# NEQ 14 >
		...
		<form id="test_drive_pop" class="form-horizontal" action="#arguments.form_action#" method="post">
			...
			<fieldset>
				...
				<div class="form-group">
					<label class="control-label col-md-3" for="Contact_Us"></label>
					<div class="col-md-6">
						<input type="submit" class="btn btn-primary" id="Contact_Us" value="#arguments.btn_label#" <cfif url.sdds eq 1> onclick="tmsomni.linkTrackEvents = 'event54';"</cfif>>
					</div>
				</div>
			</fieldset>
		</form>
...
<cfif #t_clik.track_clicks# EQ 1 >
	<cfquery datasource="Ultra10" name="g_Clk">SELECT ID,test_drive_srp_click FROM Vehicle_Click_Stats WHERE VIN='#form.VOI_Vin#' </cfquery>
	<cfif #g_Clk.recordcount# NEQ 0 ><cfset g_Clk.test_drive_srp_click = #g_Clk.test_drive_srp_click# + 1 > 
	<cfquery datasource="Ultra10" name="U_Clk">UPDATE Vehicle_Click_Stats SET test_drive_srp_click=#g_Clk.test_drive_srp_click#,last_Upd=#CreateODBCDateTime(now())#WHERE VIN='#form.VOI_Vin#'</cfquery></cfif>
</cfif> </cfoutput>    
<!--- Clicks Tracker --->
