<cfcomponent>
	...
	<cffunction name="get_legacy_nav" description="gets dealer legacy videos" output="Yes" access="public">
		...
		<cfquery datasource="Ultra8" name="Get_Legacy_Nav">
			...
			AND			Nav_Type_ID = 15
			AND			Nav_Custom_ID not in (Select Legacy_Nav_Custom_ID from Aspen10.dbo.Dealer_Videos where #arguments.Site_ID_Name# = <cfqueryparam cfsqltype="CF_SQL_INTEGER" value="#arguments.Site_ID#" maxlength="4">)
			ORDER BY 	Hierarchy
		</cfquery>
		...
		<cfif listlen(form.Nav_Custom_ID) gt 0>
			...
			<cfloop list="#form.Nav_Custom_ID#" index="myid">
				...
				<cfif len(trim(Get_Legacy_Nav.Icon_Image)) gt 0>
					<cftry>
						<cfhttp method="get" url="http://static.wdautos.com/dealer/#Get_Dealer.File_Dir#/videos/thumbs/#Get_Legacy_Nav.Icon_Image#" path="E:\inetpub\wwwroot\Aspen10\dealer\#Get_New_Dealer.Dealer_Directory#\videos" file="#Get_Legacy_Nav.Icon_Image#" timeout="60"></cfhttp>
						<cfcatch></cfcatch>
					</cftry>
				</cfif>
