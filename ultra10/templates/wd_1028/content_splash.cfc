<cfcomponent>
	...
	<!--- Check for Quick Quote / Search Links  / Video --->
		...
		<cfquery datasource="ultra10" name="Chk_Srch">SELECT Dealer_ID,use_hotlist,	srch_method,bxs_new_used, price_srch_method  FROM	Dealers WHERE Dealer_ID = #url.dealer_id#</cfquery>	
	...
	<!--- Check for Quick Quote / Search Links / Video --->
		...
		<section class="section-splash-spl">
			<div class="container">		
				<div class="container">	
			<cfif #Chk_Srch.use_hotlist# eq 1 >			<!--- Marketing Banner   01/18/2016 --->
				<cfinclude template="section_marketing.cfm" >
			</cfif>
				<div class="row pad-top-10">	
					...
					<cfif #url.dealer_id# eq 14 or  #url.dealer_id# eq 1  or  #url.dealer_id# eq 8 or  #url.dealer_id# eq 3 or  #url.dealer_id# eq 4 or  
							#url.dealer_id# eq 6 or  #url.dealer_id# eq 7 or  #url.dealer_id# eq 12 >
					<!--- new --->
						...
						<cfif #gt_search.recordcount# NEQ 0 >
							<cfloop query="gt_search">
								...
								<cfif #len(gt_search.CTA_image)# GT 0 >
									<img alt="#gt_search.CTA_Title_1#" src="http://dealers.wdautos.com/dealer/#url.assets#/images/icons/#gt_search.CTA_image#"   class="img-responsive" >
								<cfelse>
				...
				<cfelse>		
						...
						<cfset ln_1_style="font-size:#gt_search.ft_size#;color:###gt_search.ft_color#;font-weight:#gt_search.ft_weight#">
						<cfset ln_1_align="#gt_search.txt_align#">
						<cfset ln_2_style="font-size:#gt_search.ft_size_2#;color:###gt_search.ft_color_2#;font-weight:#gt_search.ft_weight_2#">
						<cfset ln_2_align="#gt_search.txt_align_2#">
						<div class="row">  
						<cfif #gt_search.recordcount# NEQ 0 >
							<div class="col-md-12 pad-top-10 brands-title visible-sm visible-xs visible-lg visible-md" style="align:#ln_1_align#">
							<span style="#ln_1_style#">#gt_search.CTA_Title_1#</span>
							<cfif #gt_search.CTA_Title_2# NEQ "">
								<br><span style="#ln_2_style#">#gt_search.CTA_Title_2#</span>
							</cfif>	
							</div>	
						<cfelse>
							<div class="col-md-12 pad-top-10 quik-link-btn"> 
...
<!--- Splash Banners --->
<cfif #url.dealer_id# EQ 1 OR  #url.dealer_id# EQ 15 >					<!--- Trucks banner 2016 --->
	...
	<cfif #Chk_Splash.recordcount# NEQ 0 >
		<section class="section-banners">
			<div class="container pad-top-10 visible-md visible-lg visible-sm visible-xs">
				...
				<div class="row-centered splash-logos-row">
...
<!--- BRANDS --->  
		...
		<cfif #url.dealer_id# eq 15>  <!--- Baierl Truck Depot   or #url.dealer_id# eq 1  --->
			<cfset nbr_col = "3" >
		<cfelse>
			<cfset nbr_col = "2" >
		</cfif>
		...
		<cfif  #Get_Assigned_OEMs.recordcount# GT 0 >
			...
			<cfset ln_1_style="font-size:#gt_brands.ft_size#;color:###gt_brands.ft_color#;font-weight:#gt_brands.ft_weight#">
			...
			<section class="section-brands">
				 <div class="container pad-top-10">
					...
					<div class="row-centered">
						 <div class="col-centered<cfif #url.dealer_id# neq 15> col-md-offset-2</cfif>" > <!--- used the offset to center the odd numbered logos 01/07/2016 --->
							<cfloop query="Get_Assigned_OEMs" >
								<div class="col-md-#nbr_col#" style="margin-right:3px;margin-bottom:10px;<cfif #url.dealer_id# eq 15>max-width:220px;</cfif>">
...
<!--- Splash Banners --->																		
<cfif #url.dealer_id# NEQ 1 AND  #url.dealer_id# NEQ 15>
	<cfquery name="Chk_Splash" datasource="Ultra10" >
		SELECT  Nav_ID, Dealer_ID, Nav_Type_ID,Start_Date, End_Date
		FROM    Navigation
		WHERE  	Dealer_ID = #url.dealer_id# 
...
<!--- CTA UPPER --->
<cfif #url.dealer_id# neq 27 >   <!--- Ohio Valley Honda goes AFTER Carousel --->
					...
					<div class="row pad-bottom-10">
						<cfloop index="x" from="1" to="#maxblk#">
							<div class="col-md-3 pad-top-5 <cfif #x# eq 1>verticalBlank<cfelse>verticalLine</cfif>"  align="center">
								...
								<cfelse>
									<div class="row  cta-upper-title" >#a_Title[x]#</div>
								</cfif>
...
<cfif #url.dealer_id# NEQ 45 AND  #url.dealer_id# NEQ 179  AND  #url.dealer_id# NEQ 38  AND  #url.dealer_id# NEQ 343  AND  #url.dealer_id# NEQ 1 >	 <!--- Fuccillo Scion & Stoltz Scion & Washington Scion --->
			...
			<div class="container">
				...
				<div class="row visible-lg visible-md">
					...
					<div class="row"><div class="col-md-12 pad-top 5px  visible-lg visible-md"><div class="page-div"></div>		
						...
						<div id="#arguments.assets#-spl-carousel" class="pad-top-10" style="border:1px solid <cfif url.dealer_id neq 15> white<cfelse> black</cfif>">
								<cfif #url.dealer_id# NEQ 1 >
									<cfset howmany = 4 >
								<cfelse>	
									<cfset howmany = 6 >
								...
								<cfinvokeargument name="minSlides" value="#howmany#"/>	
...
<!--- CTA UPPER --->
<cfif #url.dealer_id# eq 27 >  <!--- Ohio Valley Honda --->
		<cfquery name="Get_Block2" datasource="Ultra10" maxrows="4" >
			SELECT  ID, dealer_id, Title, image, free_text, button_text, button_link, target, active, create_ts, hierarchy
			FROM    Nav_Block_2
			where 	dealer_id = #url.dealer_id#
...
<cfif #url.dealer_id# NEQ 1 >
			...
			<div class="container">
				<div class="row">
					<div class="col-md-5 verticalBlank">
						<cfif #url.dealer_id# NEQ 38 and #url.dealer_id# NEQ 45 and #url.dealer_id# NEQ 179 and #url.dealer_id# NEQ 343 >
						...
						<cfelse>
							<div class="spl-search-name pad-top-10 pad-5" style="align:left"><span class="spl-search-name pad-5" style="color:white" >REQUEST MORE INFO</span></div>
						</cfif>
							<div class="row"><div class="col-md-12 visible-sm visible-xs visible-lg visible-md"><div class="gradient"></div></div></div>	
		...
		<cfelse>			
				<cfinclude template="section_about_us_collapse.cfm" >
		</cfif>	
	...
	<cffunction name="legacy" description="shows a legacy splash page" output="Yes" access="public">
		...
		<div class="section-carousel">
			<div class="container pad-top-10" style="align:center">
		...
		<div class="section-video">
			<div class="container">
				<table style="width:100%">
					<tr>
						<td style="width:50%;align:center" class="pad-10" >
							<cfinvoke component="/cfcs/ultra10/nav_elements" method="query">
								<cfinvokeargument name="Get_Nav" value="#Get_Nav#"/>
								<cfinvokeargument name="Get_Phones" value="#Get_Phones#"/>
								<cfinvokeargument name="nav_type_id" value="30"/>
