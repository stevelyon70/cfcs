		...
		<cfquery datasource="Ultra10" name="Get_Coupon">
			...
			<cfif url.dealer_id eq 70 >
				<cfif url.Department eq 12>
					<img alt="Sevice Coupons"  class="img-responsive" src="http://dealers.wdautos.com/dealer/findlaytoyota/images/library/header_service.jpg" /><br>					
				</cfif>
			</cfif>
			 
<!--- GLOBAL Coupon Box re-size 10/07/2015 --->
<cfif #url.dealer_id# NEQ 30 AND #url.dealer_id# NEQ 37  AND #url.dealer_id# NEQ 46  AND #url.dealer_id# NEQ 70 >
<style type="text/css">
	/*legacy coupons*/
	.legacy-coupon-box{
	...
	.legacy-coupon-title{text-align:center!important;}
</style>
<cfelse>
<style type="text/css">
	/*legacy coupons*/
	...
	.legacy-coupon-title{text-align:center!important;}
		...
		<cfloop query="Get_Coupon">
			...
			<div class="row legacy-coupon-box">
				...
				<div class="row">
					<div class="col-md-12" align="center" valign="TOP">
						<!--- Department&nbsp;:&nbsp; #variables.Deparment_Name# ---><br>
						Expiration&nbsp;:&nbsp;<cfif len(trim(Expires)) eq 0>Does Not Expire<cfelse>#dateformat(Get_Coupon.Expires, "mm/dd/yyyy")#</cfif><br>
					<!--- </div>
	...
	<cffunction name="modern_standard_2_up" description="shows coupons from legacy system" output="Yes" access="public">
...
<style type="text/css">
	...
	<cffunction name="full_container" description="shows coupons from legacy system" output="Yes" access="public">
		...
		<cfquery name="get_dlr" datasource="Ultra10">
        	SELECT  Dealer_ID, Display_Name, dealer_directory 
            FROM Dealers 
...
<div class="content-container coupon">
		<div class="row-centered">
        	<a name="serviceCoupons"></a><h2>Service & Parts Coupons</h2>
        </div>
		<cfloop query="Get_Coupon">
			<!--- Find the dealer that has the image in case the coupons is from a share --->				
			<cfquery name="get_dlr" datasource="Ultra10">
            	SELECT  Dealer_ID, Display_Name, dealer_directory 
                FROM Dealers 
                WHERE Dealer_ID =  #Get_Coupon.dealer_id#
            </cfquery> 
			<cfset variables.DEALER_DIRECTORY = #get_dlr.dealer_directory# >
