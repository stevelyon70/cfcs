<cfcomponent>
	...
	<cffunction name="import_options" description="Imports the tcuv options & incentive xml data from local file" output="yes" access="public">	
		...
		<cfloop query="Get_Vins">
			...
			<li>Importing Options for #Get_Vins.Vin#</li>
			...
			<cfelse>
				...
				<ul>
					<li>Incentive ID: #variables.incentive_id#</li>
					<li>Incentive APR: #variables.incentive_apr#</li>
					<li>Disclaimer: #variables.disclaimer#</li>
					<li>Installed: #variables.installed_list#</li>
					<li>Safety: #variables.safety_list#</li>
					<li>Exterior: #variables.exterior_list#</li>
					<li>Interior: #variables.interior_list#</li>
					<li>Mechanical: #variables.mechanical_list#</li>
				</ul>
	...
	<cffunction name="fetch_options" description="fetches the tcuv options xml data" output="yes" access="public">
		...
		<cfloop query="Get_Vins">
			...
			<li>Fetching Options for #Get_Vins.Vin#</li>
		...
		<cfelse>
			...
			<cfif variables.vehicle_count gt 0>
				...
				<cfloop from="1" to="#variables.v_array_len#" index="vid">
					...
					<cfquery datasource="Imports" name="Get_Comments">
						SELECT     	Comments
					...
					<cfquery datasource="Imports" name="Insert_tcuv">
						...
						VALUES
							(	<cfqueryparam cfsqltype="cf_sql_integer" value="#arguments.dealer_id#" maxlength="10">,
								...
								<cfif len(trim(Get_Comments.Comments)) gt 0><cfqueryparam cfsqltype="cf_sql_varchar" value="#Get_Comments.Comments#"><cfelse>NULL</cfif>
							)
					</cfquery>
	...
	<cffunction name="fetch" description="gets the tcuv xml data" output="yes" access="public">
		...
		<cfhttp method="post" url="https://eigws.toyota.com/Vehicle/UsedInventory/lookup" result="httpResponse" timeout="60">
			<cfhttpparam type="header" name="accept-encoding" value="no-compression"/>
			<cfhttpparam type="xml" value="#trim(tcuvXML)#"/>
		</cfhttp>
		<!--- <cfoutput>RESPONSE: #httpResponse.fileContent#</cfoutput><cfabort> --->
				...
				<cfquery datasource="Ultra8" name="Insert_tcuv">
					...
					VALUES
						(	<cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.dealercode#" maxlength="30">,
							...
							<cfif len(trim(Get_Vins.Stock)) gt 0><cfqueryparam cfsqltype="cf_sql_varchar" value="#Get_Vins.Stock#" maxlength="50"><cfelse>'n/a'</cfif>,
							...
							1,
							<cfif len(trim(Get_Vins.Dealer_Comments)) gt 0><cfqueryparam cfsqltype="cf_sql_varchar" value="#Get_Vins.Dealer_Comments#"><cfelse>NULL</cfif>
						)
				</cfquery>
	...
	<!--- Update Ultra10 with the certified vehicles --->
	 <cffunction name="update_ultra10" description="Updates ultra 10 ceritified vehicles" output="yes" access="public">

		<!--- arguments --->
		<cfargument name="dsn1" required="true">
		<cfargument name="dsn2" required="true">
		
		
		<cfquery datasource="Ultra10" name="Update_Ultra10_Certified">
			UPDATE		Vehicles
			SET			Certified=0
			WHERE		V_New_Used = 'U'
			AND			V_Make = 'Toyota'
			AND 		Certified = 1
			AND			dealer_ID <> 70   <!--- EXCLUDE FINDLAY --->
		</cfquery>
		
		
		<cfquery name="getToyotaDealers" datasource="#arguments.dsn1#">
		SELECT		Dealership_Name,
					Certified,
					Vin
		FROM		Inventory
		WHERE		Make = 'Toyota' AND New_used = 'U' AND Certified = 1

		</cfquery>

		<cfif getToyotaDealers.RecordCount NEQ 0>
		<ul>
			<cfloop query="getToyotaDealers">
				<cfset vinnum = getToyotaDealers.Vin>
				<cfoutput>U10 - #getToyotaDealers.Dealership_Name# - #getToyotaDealers.Vin#<br></cfoutput>
				<cfquery name="updateCertified" datasource="#arguments.dsn2#">
					UPDATE Vehicles 
					SET Certified = 1
					WHERE Vin = '#vinnum#'
				</cfquery> 
		...
		<cfset updated_dlrs = #getToyotaDealers.RecordCount# >
		
		<cfreturn updated_dlrs>
